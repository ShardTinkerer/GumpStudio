...
using GumpStudio.Plugins.RunUo;

using Xunit;

namespace GumpStudio.Plugins.RunUo.Tests;
...
public class RunUoExportTests
{
    ...
    {
        RunUoExporterPlugin plugin = new();
        RecordingHost host = new();

        plugin.Initialize(host);

        Assert.Equal("gumpstudio.exporters.runuo", plugin.Info.Id);
        Assert.Equal(2, host.Exporters.Count);
        Assert.Equal(["runuo", "runuo-numeric"], host.Exporters.Select(e => e.Id));

        IGumpExporter exporter = host.Exporters[0];

        Assert.Equal(".cs", exporter.FileExtension);
        Assert.Contains(
